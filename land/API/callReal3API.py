...
def save_to_sqlite(df: pd.DataFrame, engine, table: str):
    sql = f"delete from {table} where 법정동시군구코드 ='{lawd_cd}' and 계약년도 = '{deal_ymd[:4]}' and 계약월 = '{int(deal_ymd[4:])}'"   
    ...
    if not df.empty:
        ...
        print(f"✅ tbl_real_estate : {lawd_cd},{deal_ymd} 저장 완료 ({len(df)}건)")
    else:
        print(f"⚠️ tbl_real_estate : {lawd_cd},{deal_ymd} 저장할 데이터 없음")
...
def insert_into_tblReal(engine,lawd_cd, deal_ymd):
    ...
    sql = f"delete from tblReal where sggCd ='{lawd_cd}' and dealDate between '{start_date}' and '{end_date}'"   
    ...
    sql = """
    INSERT INTO tblReal(	sggCd 	,aptSeq     ,excluUseAr     ,dealDate 	 	,floor 	
...
def insert_into_tblRealStatUnitMon(engine,lawd_cd, deal_ymd):
    ...
    sql = f"delete from tblRealStatUnitMon where sggCd='{lawd_cd}' and dealYear = '{year}' and dealMon =  '{month}' "   
    ...
    sql = f"""
            insert or replace into tblRealStatUnitMon
...
if __name__ == '__main__':
    try:    
        df_input = pd.read_sql("select code, p1_code from tblCode where cha = 3 and isUsed in (1) order by code", con=engine)
        ...
        for idx, row in df_input.iterrows():
            ...
            #for deal_ymd in [sBefMon, sCurMon ]:  # 현재 월과 이전 월만 처리
